  ...
  const handleServiceFeeChange = (serviceId, fee) => {
        setFormData(prevData => {
            ...
            if (isNaN(newFee) || newFee < 0) {
                 // If the input is cleared or invalid, just remove it from the array
    ...
    // Helper function to get the current fee for a service to display in the input
...
<div className="space-y-6">
    ...
    {formData.department === 'Doctor' && (
      <div>
        ...
        >
          ...
          {specializations['Doctor']?.map(spec => ( // Assuming specializations is an object
  ...
  {/* ==================================================================== */}
  {/* ✅ NEW: Conditionally Rendered Doctor's Fees Section */}
  {/* ==================================================================== */}
  {formData.department === 'Doctor' && (
    <div className="p-4 border-t border-b border-gray-200 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Dynamically create an input for each service type */}
            {availableServices.map(service => (
                <div key={service.id}>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                        {service.service_name} Fee ($)
                    </label>
                    <input
                        type="number"
                        min="0"
                        step="0.01"
                        // Get the current value from our helper
                        value={getServiceFee(service.id)}
                        // Use the new, specialized handler
                        onChange={(e) => handleServiceFeeChange(service.id, e.target.value)}
