...
**Reconnect never counts against the plan (decided with the user).** Every Meta connect route (Facebook/Instagram connect + quick connect, Instagram sync-from-Facebook, WhatsApp Embedded Signup) matches the account by workspace + Page / IG account / phone-number id and updates that same row — bot id and all its data kept. `max_bot_accounts` is checked only for accounts that would be a NEW row (`utils/botAccountLimit.js` `countNewAccounts` / `assertRoomForNewAccounts`). A multi-account import whose new accounts don't fit is refused as a whole before anything is saved or subscribed (the import screens call `POST /channels/import-check` first). This is how customers move to a new Meta app / Business: re-import, same bots. Check: `npm run test:reconnect-limit` (read-only).
...
## Broadcasting workflow (Flow Builder) + Message Template element

Create on the Broadcasting page (`/campaigns`: name + bot account + **broadcast type** — WhatsApp only, Anytime (default) or Inside 24 hours; other channels are always WINDOW) → `POST /broadcasts/start-with-flow {mode}` creates the flow **with the campaign's `integration_id`** and the matching element (Message Template for TEMPLATE, Send Message for WINDOW) already connected to the Broadcast element → the Flow Builder opens with the Broadcast element's settings showing. The account is fixed from then on (read-only in the element and in Configure and Send).
- **Broadcast element** (`Components/Broadcast/BroadcastStartNodeProperties.jsx`): name, Sending Mode (WhatsApp) Anytime = `mode TEMPLATE` (a **Message Template** element connected) / Inside 24 Hours = `mode WINDOW` (a message element) — switching calls the builder's `handleSetBroadcastFirstStep`, which reuses or adds the element and swaps only the Broadcast element's own edge — and audience (`AudienceForm.jsx`). Only the standard **Done** button; no save/send here. Campaign state lives in `useBroadcastCampaign.js` (context shared with the builder top bar); audience edits autosave to the campaign (800 ms) and are flushed on Back.
- **Builder top bar on a broadcast flow** (decided with the user — a save never sends and never leaves the builder): **Save** = flow + campaign draft, then a non-blocking toast (audience count, or the first `readyErrors` item). **Review & Send** saves, then opens `BroadcastSendDialog.jsx` (account, type, server audience count, readiness, Send now / Schedule / Reschedule) → `confirmBroadcastAudience` → send/schedule → `showSendStarted` → redirect to `/campaigns`. Back falls back to `/campaigns`. Dialogs + SweetAlerts live in `Components/Broadcast/broadcastDialogs.js` (shared with the list page).
- **Counters** (`sent/delivered/read/failed_count`) are always recounted from `broadcast_logs` (one row per subscriber) by `utils/broadcastStats.js` `recountBroadcastStats` — never incremented. Sent = SENT/DELIVERED/READ, Delivered includes Read, a message accepted then reported failed moves from Sent to Failed. Both the runner and the status webhook call it. It also emits `broadcast_update` (status + counters) to `agency:<id>` via `emitBroadcastUpdate`, as do the runner's status changes and schedule/cancel; `CampaignListPage.jsx` merges it into the list live (reloads on socket reconnect).
- `PUT /broadcasts/:id` only changes the audience fields sent (`idsOrExisting`); stored include/exclude contact ids are loaded back into the pickers (`resolveContacts`) so a save never clears them.
- **Audience** (`utils/broadcastRunner.js` `computeAudience`): only the campaign's bot account's subscribers (a conversation on it), `SUBSCRIBED`, not blocked. The same function counts the preview, the confirmation dialogs and the real send.
- **Server gate** (`routes/broadcasts.js` `gateSend` → `inspectCampaign`): send / schedule re-check the account, the content for the mode (WINDOW: a sendable message; TEMPLATE: a Message Template element right after the Broadcast element with an approved template of **that** account and every parameter filled, or a flow-less campaign's own `template_id`) and the audience. An unfiltered audience or one ≥ `BROADCAST_LARGE_AUDIENCE_THRESHOLD` (env, default 5000) needs `confirmAudience: true` or answers 409 `AUDIENCE_CONFIRMATION_REQUIRED`. `PUT` returns `audienceCount/noFilter/readyErrors`.
- **Schedule**: `POST /broadcasts/:id/schedule` schedules DRAFT/FAILED and **reschedules** SCHEDULED on the same row (never a second schedule). A draft may keep a planned `scheduled_at` (shown as "Planned"); only `status = SCHEDULED` is picked up by the scheduler. Labels shown: Draft · Scheduled · Sending · Sent · Failed · Cancelled. **A WhatsApp Inside 24 hours (WINDOW) broadcast can't be scheduled** (decided with the user) — `canSchedule` in `routes/broadcasts.js` (400 `SCHEDULE_NOT_ALLOWED`; a SCHEDULED one can't switch to WINDOW; a draft switching to it drops its planned time), mirrored by `canScheduleBroadcast` in `broadcastDialogs.js` (both send screens show Send now only).
- **Message Template element** (node type `whatsappTemplate`, WhatsApp only; palette of flows **and** Sequences): `utils/templateMessage.js` on the server (flowEngine, sequenceRunner, broadcastRunner all use it; `messagingWindow.canSendNow` always allows it), `Components/Templates/MessageTemplateFields.jsx` + `messageTemplateUtils.js` in the UI. Only the flow's own account's approved templates; parameters accept `{{contact.name}}`-style variables. Template parameter builders were moved from `commerceEvents.js` to `utils/whatsappTemplateParams.js` (re-exported there).
  - **No Delay / "typing…"** on this element (decided with the user): excluded in the UI (`DELAY_EXCLUDED_NODE_TYPES`, not in `TYPING_ELIGIBLE_NODE_TYPES`) and an old saved `data.delay` is ignored by flowEngine.
  - **Media header** (image/video/document): `params.headerMedia` (upload, URL or `{{variable}}`), else the template's stored sample `header_media_url`; with neither it's a missing parameter (UI validation + `missingTemplateParams`). `platformSender` turns a relative `/uploads/…` link into a public URL before sending.
  - **Quick-reply buttons have the full flow-button options** (next step via their own `btn-<templateIndex>` wire, Go to Flow, add/remove labels, enroll/remove Sequence) — `data.buttons[i]` aligned with the template's button index (`syncTemplateButtons`), edited with `ButtonActionEditor variant="templateButton"` (fixed title, not removable). Each quick reply is sent with the `FBTN` routing token (`buildTemplateSend(..., { routeFor })`), so taps route through flowEngine step 0 like any button. The element's plain continuation is only its Next Step wire; broadcast walks skip option wires (`isOptionHandle`). Not in Sequences (taps aren't routed there — the UI says so).
- Test: `npm run test:broadcast -- <windowAccountId> <templateAccountId> <templateId>` (real DB, never sends, cleans up). Unit: `test/templateMessage.test.js`.

## Comment Automation: per-post copies, saved campaigns, paged posts

- **A post's campaign is always its own** (decided with the user — reversed an earlier "shared campaign" version): **Automate → Use a copy** copies an existing campaign into a NEW campaign of that post (`POST /comments/campaigns/:id/copy`, `copyCampaign` in `utils/commentRulePosts.js`: all reply/DM/moderation settings, counters from 0), so editing one post's replies never changes another's. A post runs at most one campaign per bot account (409 `POST_ALREADY_AUTOMATED`); `comment_rule_posts` (`migrate_comment_rule_posts.js`) holds each post campaign's post and **matching always goes through it** (`findRuleForPost`, used by `GET /comments/posts` and the comment webhook, which keeps its loose id match).
- **Saved campaigns**: `post_id = 'SAVED_CAMPAIGN'` — a reusable copy kept in the list that runs on no post (the webhook query excludes it; `findRuleForPost` skips it). Created with "Also save as a reusable campaign" when making a post's own campaign (`saveAsCampaign` on `POST /comments/campaigns` — needs room for 2 in `max_comment_rules`; every copy and saved campaign counts).
- **Only a campaign of the post's own Page / Instagram account** can be used (its replies are written for that page): `postBelongsToAccount` (Facebook: post id starts with `<pageId>_`; Instagram / short ids: Meta's `owner` / `from`; any doubt = refused, 400 `POST_OF_OTHER_ACCOUNT`), and the chooser only lists that account's campaigns.
- `DELETE /comments/campaigns/:id` (was missing — the UI's delete always failed; its link cascades). The post card's ✕ deletes that post's (own) campaign. Copy needs `comment_automation.create` for team members.
- **Reply image**: `auto_reply_media_url` is usually an uploaded `/uploads/…` path; the comment webhook sends it to Meta as a full public link (`resolvePublicImageUrl` + `getPublicBackendUrl`, like WhatsApp media) — the bare path made Meta refuse the whole public reply. If Meta still refuses the image, the text reply is sent without it, and Meta's error is logged.
- Posts & Reels are paged **12 per page with page numbers** (`utils/metaPosts.js`): the latest 1,000 post ids are listed (ids only, cached 2 min, page 1 always fresh) for the total, then the page's posts are fetched in one Graph **batch** request — the `?ids=` multi-lookup is deprecated (v26+). Meta's own "after" cursors can be 1,000+ chars.
...
- **Connect** (`routes/commerce.js`, UI `Components/Commerce/StoreConnectionsManager.jsx`, shown in Settings → App Integrations → Store API *and* Automation → Commerce → Store Connections): Shopify = store subdomain + Admin API token (`shpat_`, never expires) **or** Dev Dashboard client id + secret (client-credentials grant, 24h token refreshed by `getShopifyToken`). A Dev Dashboard app must also be granted **protected customer data** access (Name/Email/Phone/Address) or Shopify refuses every order query ("not approved to access the Order object"); admin-created (`shpat_`) apps have it automatically. The Store Connections table shows fix-it hints for this and for missing scopes. WooCommerce = store URL + REST consumer key/secret (Read/Write). Credentials are verified live, then stored encrypted; many stores per workspace. Connect/disconnect is owner-only. The old Partner-app OAuth flow (unsigned `state` = agency id) was removed on purpose. In production a Woo URL must be HTTPS and must not resolve to a private IP (SSRF guard in `normalizeWooUrl`).
