...
const HospitalStaffs = () => {
    ...
    const [teamMembersPerPage] = useState(15); // You can adjust this value
    ...
    try {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'Failed to fetch teamMembers. Please ensure the server is running and try again.',
            customClass: {
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await fetch(`http://localhost:5000/team-members${id}`, {
                        method: "DELETE",
                    });
                    ...
                    if (res.ok) {
                        ...
                        setCurrentPage(prevPage => {
                            ...
                            const maxPage = Math.ceil(totalFiltered / teamMembersPerPage);
    ...
    // Edit patient
    const handleEdit = (id) => {
        navigate(`/teamMembers/edit/${id}`);
    };

    // View patient details
    const handleView = (id) => {
        const item = teamMembers.filter(p => p.id == id)
    ...
    const filteredPatients = teamMembers.filter((p) => {
        const contact = p.phone ? p.phone.toString().toLowerCase() : "";
        const term = search.toLowerCase();
        return contact.includes(term);
    });

    // Get current teamMembers for pagination
    const indexOfLastPatient = currentPage * teamMembersPerPage;
    const indexOfFirstPatient = indexOfLastPatient - teamMembersPerPage;
    const currentPatients = filteredPatients.slice(indexOfFirstPatient, indexOfLastPatient);

    // Calculate total pages
    const totalPages = Math.ceil(filteredPatients.length / teamMembersPerPage);
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [search, teamMembers]); // Also reset if teamMembers data changes (e.g., after delete)
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {loading ? (
                                    ...
                                    currentPatients.map((patient, index) => (
                                        ...
                                        >
                                            <td className="p-4 text-gray-700 text-base">{(currentPage - 1) * teamMembersPerPage + index + 1}</td>
                                    ...
                                    <tr>
                                        <td colSpan="6" className="text-center py-10 text-gray-500 text-xl">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <p className="font-semibold">No teamMembers found.</p>
